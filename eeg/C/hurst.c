...
float avg_scaled_range(uint32_t np, int32_t *x, int32_t n)
{
    ...
    #pragma omp parallel for private(i) reduction(+:M)
    for (r = 0; r < np; r += n) {
        // Calculate mean
        M = 0;
        for (i = 0; i<n; i++) {
...
void hurst(uint32_t np, int32_t *x, float *h)
{
	...
	float *graphx = malloc(steps * sizeof(float));
	float *graphy = malloc(steps * sizeof(float));
  ...
  #pragma omp parallel for reduction(+:Sumx, Sumy, Sumxx, Sumxy)
	for (i = 0; i < steps; i++) {
		Sumx += graphx[i];
		Sumy += graphy[i];
		Sumxy += (graphx[i]) * (graphy[i]);
		Sumxx += (graphx[i]) * (graphx[i]);
	}
